...
import { registerSchema, type registerSchemaType } from "../../libs/formValidation";
...
export default function Register() {
    ...
    return (
        <form className="shadow rounded-md bg-base-200 py-6 px-4 w-full space-y-4" onSubmit={handleSubmit(onFormSubmit)}>
            <h1 className="font-semibold text-center">Register an account</h1>
            ...
            <fieldset className="fieldset">
                ...
                <input type="email" className="input w-full" placeholder="Your email" {...register("email")} />
                {errors?.username && <p className="text-red-500 label">{errors?.username?.message}</p>}
            </fieldset>
            <fieldset className="fieldset">
            ...
            <p className="text-sm">Already have an account?
                <Link to="/login" className="font-semibold mx-1">Login</Link>
            </p>
        </form>
    )
}
